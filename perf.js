/* ================================================= */
/* === PERF.JS - FINAL MASTER V13 (FULL FEATURED) === */
/* ================================================= */
...
window.PerfLogic = {
    ...
    // =================================================
    init: function() {
        console.log("PerfLogic V13 Init - Full Version");
        this.renderTrackList(); // Baut das HTML auf
        this.setupGlobalClicks(); // Startet die Klick-Überwachung für Tabs
    },

    // --- NAVIGATION LOGIC (STABILSTE METHODE) ---
    setupGlobalClicks: function() {
        // Überwacht jeden Klick auf dem Bildschirm
        document.body.addEventListener('click', (e) => {
            // Prüft, ob ein Navigation-Button (oder dessen Icon/Text) getroffen wurde
    ...
    switchTab: function(tabName) {
        ...
        // 4. Analytics Daten laden, falls nötig
    ...
    toggleTrackSelection: function(track) {
        if(this.selectedTrackId === track.id) this.deselectTrack(); else this.selectTrack(track);
    },
    selectTrack: function(track) {
        ...
        if(this.isCreatorMode) {
            ...
            console.log("Details: " + track.name);
        }
    },
    ...
    enterCreatorMode: function() {
        ...
        setTimeout(() => { 
            if(this.map) { 
                this.map.invalidateSize(); this.map.dragging.enable(); this.map.touchZoom.enable(); 
    ...
    clearCreatorMap: function() {
        if(this.creatorPoints) this.creatorPoints.forEach(p => { if(p.marker) this.map.removeLayer(p.marker); });
        this.creatorPoints = [];
    ...
    finalizeSave: function() {
        ...
        const track = {
            id: Date.now(), name: name, routePath: this.currentRouteGeo, sectors: this.currentSectorsData || [], 
            pins: this.creatorPoints.map(p => ({lat: p.latlng.lat, lng: p.latlng.lng, type: p.type})),
            dist: this.currentRouteStats.dist, elevUp: this.currentRouteStats.elevUp || "0m", elevDown: this.currentRouteStats.elevDown || "0m",
            bestTime: '---', config: { type: this.startType, flyTarget: targetSpeed, flyMin: document.getElementById('fly-min').value, flyMax: document.getElementById('fly-max').value }
        };
    ...
    renderTrackList: function() {
        ...
        let html = `
            ...
            <div id="view-drag" style="display:none; padding:40px; text-align:center;">
                <h3 style="color:#444; font-weight:900;">DRAG MODE</h3><p style="color:#666;">COMING SOON</p>
            </div>
            ...
            <div class="perf-sub-nav">
                <div class="psn-item active" id="btn-tab-track" data-tab="track">TRACK</div>
                <div class="psn-item" id="btn-tab-drag" data-tab="drag">DRAG</div>
                <div class="psn-item" id="btn-tab-analytics" data-tab="analytics">ANALYTICS</div>
            </div>
        `;
        ...
        this.tracks.forEach((t, index) => {
            ...
            const dist = t.dist || "0.0 km"; const name = t.name || "UNNAMED TRACK"; const time = t.bestTime || "---";
            div.innerHTML = `<div class="tc-bg-map" id="mini-map-${t.id}"></div><div class="tc-content"><div class="tc-name">${name}</div><div class="tc-details"><span><i class="fa-solid fa-road"></i> ${dist}</span><span><i class="fa-solid fa-stopwatch"></i> ${time}</span></div></div><div class="tc-condition-badge" id="cond-badge-${t.id}" style="display:none"></div>`;
        ...
        // HIER KEIN TIMEOUT MEHR NÖTIG, WEIL WIR GLOBAL LISTENER NUTZEN
        this.updateLiveDashboard();
    },
