/* GARAGE.JS - FINAL V12 (CLEAN RESTART) */
...
window.GarageLogic = {
    ...
    // 3. START
    init: function() {
        console.log("Garage V12 Init");
        this.renderCars();
    },

    // 4. HAUPTFUNKTION (DASHBOARD)
    renderCars: function() {
        this.renderCarCard();
        this.renderDriveCard();
    },
    
    // Dummy gegen App-Absturz
    renderList: function() { },
    ...
    openEditor: function(index) {
        ...
        if(!overlay) { 
            alert("Fehler: Overlay HTML fehlt!"); 
            return; 
        }
        
        document.body.appendChild(overlay); // Nach vorne holen
        overlay.style.display = 'flex';
        overlay.classList.remove('hidden');

        // 1. MODELLE GENERIEREN
            ...
            this.availableModels.forEach(m => {
                ...
                div.onclick = () => {
                    document.querySelectorAll('.model-option').forEach(e => {
                        e.style.borderColor='#444'; e.style.color='#888'; e.classList.remove('selected');
                    });
                    div.classList.add('selected'); 
                    div.style.borderColor='#007aff'; div.style.color='white';
                    
                    if(previewViewer) {
                        previewViewer.style.display = 'block';
                        previewViewer.src = m.file;
                    }
                };
                modelList.appendChild(div);
            });
        } else {
            console.error("Modell-Liste nicht gefunden!");
        }
        ...
        if(index > -1 && this.cars[0]) {
             ...
             if(previewViewer) { previewViewer.src = c.model; previewViewer.style.display='block'; }
    ...
    // WICHTIG: Das hier wird aufgerufen, wenn du auf das X klickst
